...
/*
 * If there are multiple suppliers(neighbours having the given chunk) of a given nextunsetchunk of host,
 * the neighbour with max free bandwidth is chosen. It fills up the requesting window atleast with 15 frames and
 * the number of frames with no suppliers should not exceed req_window by 4. Chunk_Req msgs are generated and
 * sent to lower tiers
 */
...
/*
 * Similar to coolstreaming with some more constraints.
 */
...
/*
 * Chooses a random neighbor. Will collect unset chunks upto 5 times the chunk size. If meanwhile, there
 * is a chunk with no supplier, it will break.
 */
...
// FIFO with some schedule setting
